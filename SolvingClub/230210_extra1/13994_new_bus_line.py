import sys
sys.stdin = open('new_bus_input.txt')
...
for tc in range(Test_case):
    ...
    A = []
    B = [0] * 1001
    for ln in range(Bus_line):
        Bus_line_number = list(map(int, input().split()))
        start = Bus_line_number[1]
        end = Bus_line_number[2]
        # print(f'#{tc+1} {Bus_line_number}')
        if Bus_line_number[0] == 1:
            ...
            if start % 2 == 0:
                for k in range(start, end+1):
                    if k % 4 == 0 or k == start or k == end:
                        A.append(k)
            if start % 2 != 0:
                for k in range(start, end+1):
                    if k % 3 ==0 and k % 10 !=0 or k == start or k == end:
                        A.append(k)
